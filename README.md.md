...
# Finding the counts
counts <- xml2::xml_find_first(website, '//*[@id="search-results"]/div[2]/div[1]/div[1]/h3/span')
...
``` r
abstracts <- gsub("\\s+", " ", pub_char_list)
abstracts <- str_extract(abstracts, "<AbstractText.*</AbstractText>")
...
*4 articles do not have abstracts.*

Now, the title

``` r
titles <- gsub("\\s+", " ", pub_char_list) 
titles <- str_extract(titles, '<ArticleTitle.*</ArticleTitle>')
titles <- str_remove_all(titles, '<(/?)ArticleTitle>')

sum(titles == '[Not Available].')
```

- How many of these don’t have a title ?

*1 article does not have a title.*
